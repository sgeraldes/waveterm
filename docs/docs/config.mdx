...
SG Wave's configuration files are located at `~/.config/waveterm/`.
...
The easiest way to edit your config files is to use the wsh editconfig command which will open your SG Wave config file in the built-in preview editor.
...
| app:dismissarchitecturewarning       | bool     | Disable warnings on app start when you are using a non-native architecture for SG Wave. For more info, see [Why does SG Wave warn me about ARM64 translation when it launches?](./faq#why-does-sg-wave-warn-me-about-arm64-translation-when-it-launches).              |
...
| ai:name                              | string   | string to display in the SG Wave AI block header                                                                                                                                                                                                                 |
...
| ai:proxyurl                          | string   | HTTP proxy URL for AI API requests (does not apply to SG Wave cloud AI)                                                                                                          |
...
```json
{
    ...
    "web:defaulturl": "https://github.com/sgeraldes/waveterm",
...
If you installed SG Wave pre-v0.9.0 your configuration file will be located at
...
To avoid putting secrets directly in config files, SG Wave supports environment variable resolution using `$ENV:VARIABLE_NAME` or `$ENV:VARIABLE_NAME:fallback` syntax. This works for any string value in any config file (settings.json, presets.json, ai.json, etc.).
...
```json
{
  ...
  "bookmark@sgwave": {
    "url": "https://github.com/sgeraldes/waveterm",
    "title": "SG Wave",
    "display:order": -1
  },
  "bookmark@sgwave-releases": {
    "url": "https://github.com/sgeraldes/waveterm/releases",
    "title": "SG Wave Releases",
...
Here is an example of defining a full terminal theme. All of the built-in themes are defined here: https://github.com/sgeraldes/waveterm/blob/main/pkg/wconfig/defaultconfig/termthemes.json (if you'd like to add a popular terminal theme, please submit a PR!)
...
SG Wave allows setting a custom global hotkey to open your most recent window from anywhere on your computer. This has the name `"app:globalhotkey"` in the `settings.json` file and takes the form of a series of key names separated by the `:` character.

### Examples

As a practical example, suppose you want a value of `F5` as your global hotkey. Then you can simply set the value of `"app:globalhotkey"` to `"F5"` and reboot SG Wave to make that your global hotkey.
