...
### v0.99.0 &mdash; Apr 3, 2026

**SG Wave fork branding, shell-header UX, widget polish, and release preparation**

- Rebranded the app, onboarding, About dialog, and packaged product as **SG Wave**
- Updated first-run links and default homepage to **https://github.com/sgeraldes/waveterm**
- Refreshed sidebar widget defaults with colored icons for a more polished out-of-box look
- Added middle-click close support for **maximize-mode block tabs only**
- Improved terminal header behavior:
  - shell icon only on the left
  - shell-provided title shown in the middle
  - fallback title until the shell sets one
  - inline double-click title override with reset-to-shell behavior when cleared
- Fixed incorrect PowerShell fallback on macOS by resolving the effective default shell from detected/configured profiles
- Continued fork-specific UI cleanup by removing Discord-first messaging and replacing it with fork/upstream project links
- Rebuilt packaged macOS app for local release validation
...
- See the new [Wave AI Modes documentation](./waveai-modes) for configuration examples and setup guides
...
Wave AI is in active beta with included AI credits while we refine the experience. BYOK (Bring Your Own Key) will be available once we've stabilized core features and gathered feedback on what works best. For current fork feedback, use the [SG Wave repository](https://github.com/sgeraldes/waveterm).

For more information and upcoming features, visit the local [SG Wave AI documentation](./waveai).
...
* WIP: Integrating new Tsunami widget framework to make writing and running widgets easier
...
- Global hotkey support [docs](./config#customizable-systemwide-global-hotkey)
- Added configuration to override the font size for markdown, AI-chat, and preview editor [docs](./config)
...
- Lots of new documentation, including new pages for [Getting Started](./gettingstarted), [AI Presets](./ai-presets), and [wsh overview](./wsh).
- Other bug fixes, performance improvements, and dependency updates
...
- `wsh setvar/getvar` to get and set variables -- [Docs](./wsh-reference#getvarsetvar)
- `wsh file` operations (cat, write, append, rm, info, cp, and ls) -- [Docs](./wsh-reference#file)
...
New minor release with bug fixes and new features! Fixed the bug around making Wave fullscreen (also affecting certain window managers like Hyprland). We've also put a lot of work into the docs, including documenting how [Widgets](./widgets) and [Presets](./presets) work!
...
- added support for Azure AI [See FAQ](./faq#how-can-i-connect-to-azure-ai)
...
- New "help" view -- uses an embedded version of the documentation site
