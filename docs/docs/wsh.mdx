...
The `wsh` command provides SG Wave Terminal's core command line interface, allowing users to interact with both terminal and graphical elements from the command line. This guide covers the basics of using `wsh` and its key features.

See the [wsh reference](/wsh-reference) for a list of all wsh commands and their arguments.

## Overview

At its core, `wsh` enables seamless interaction between your terminal commands and SG Wave's graphical blocks. It allows you to:
...
`wsh` provides direct interaction with SG Wave's graphical blocks through the command line. For example:
