...
:::warning Fork Notice
**This fork has telemetry disabled.** All telemetry collection and reporting has been removed from this version of SG Wave Terminal. SG Wave AI cloud modes work without requiring telemetry to be enabled. The documentation below is retained for reference only and describes the upstream behavior.
:::
...
For the complete, current list of all telemetry events and properties, see the source code: [telemetrydata.go](https://github.com/sgeraldes/waveterm/blob/main/pkg/telemetry/telemetrydata/telemetrydata.go)
...
If you need to discuss historical upstream telemetry behavior, use the fork issue tracker and include relevant context instead of emailing an upstream support address.

## Privacy Policy

This page is retained for upstream historical reference only; SG Wave itself does not ship telemetry collection.
